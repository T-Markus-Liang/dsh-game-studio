...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding." Then confirm with `通过宿主 ask 机制向用户提问`: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use `通过宿主 ask 机制向用户提问` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.

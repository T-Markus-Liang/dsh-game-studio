...
Read `assets/docs/technical-preferences.md` if it exists — extract performance budgets and engine for asset standard constraints.
...
Use `the host's ask/question mechanism` with two tabs:
...
Present the art-director's draft to the user. Use `the host's ask/question mechanism`:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — use `the host's ask/question mechanism` to let the user decide.
...
- **`technical-artist`**: Engine-specific hard constraints — poly count budgets per asset category, texture memory limits, material slot counts, importer constraints, anything from the performance budgets in `assets/docs/technical-preferences.md`
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via Task using gate **AD-ART-BIBLE** (`assets/docs/director-gates.md`).
...
- Does `assets/docs/technical-preferences.md` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
...
Use `the host's ask/question mechanism` for next steps. Only include options that are genuinely next based on the state check above:

...
Validates `assets/skills/*/SKILL.md` files for structural compliance and
...
- `static all` → run 7 structural checks on all skills (Glob `assets/skills/*/SKILL.md`)
...
**FAIL** if  includes `Write` or `Edit` but no ask-before-write language is found.
...
agent-evaluated reasoning check, not code execution.
...
Glob `assets/skills/*/SKILL.md` to get the complete list of skills.

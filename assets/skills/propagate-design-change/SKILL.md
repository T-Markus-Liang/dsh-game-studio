...
Spawn `technical-director` via Task using gate **TD-CHANGE-IMPACT** (`assets/docs/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `the host's ask/question mechanism` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`

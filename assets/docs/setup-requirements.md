...
| **DSH** | AI agent CLI | `npm install -g @anthropic-ai/claude-code` |
...
- Hooks use `bash assets/hooks/[name].sh` — this works on Windows because
  DSH invokes commands through a shell that can find `bash.exe`
...
DSH works with any editor, but the template is optimized for:
- **VS Code** with the DSH extension
- **Cursor** (DSH compatible)
- Terminal-based DSH CLI

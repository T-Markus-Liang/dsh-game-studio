...
Skills with `model: haiku`: `/game help`, `/sprint-status`, `/story-readiness`, `/scope-check`,
...
Spawned via `Task` within a single DSH session. Used by all `team-*` skills
...
Multiple independent DSH *sessions* running simultaneously, coordinated

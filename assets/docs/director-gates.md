...
Set once during `/game start`. Edit the file directly to change it at any time.
...
- Gate: [GATE-ID] (see assets/docs/director-gates.md)
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via `the host's ask/question mechanism` — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
`.dsh/game-studio/state/active.md` as appropriate.
...
- Game pillars and core fantasy
- Platform and performance constraints (from `assets/docs/technical-preferences.md` if configured)

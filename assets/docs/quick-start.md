...
This is a complete DSH agent architecture for game development. It
...
| `/game help` | Context-aware "what do I do next?" — reads your current phase and artifacts |
...
Templates are in `assets/docs/templates/`:
...
Also in `assets/docs/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
1. **Run `/game start`** (or `/brainstorm open`) — guided creative exploration:
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   ...
   - Creates `assets/docs/technical-preferences.md` with naming conventions,
...
1. **Run `/game start`** (or `/project-stage-detect`) — analyzes what exists,
...
assets/
  settings.json                    -- DSH hooks and project settings
  ...
  docs/
    ...
    workflow-catalog.yaml          -- 7-phase pipeline definition (read by /game help)

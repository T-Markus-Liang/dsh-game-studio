...
1. **Ask clarifying questions:**
   ...
   - *Use `the host's ask/question mechanism` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `the host's ask/question mechanism` to capture the decision*
...
- Batch up to 4 independent questions in a single `the host's ask/question mechanism` call
...
- When running as a Task subagent (tool may not be available) — structure your
  text output so the orchestrator can present options via the host's ask/question mechanism
...
**Example — multi-question batch for clarifying questions:**
  ...
  the host's ask/question mechanism with questions:

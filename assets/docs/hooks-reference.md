...
Hooks are configured in `assets/settings.json` and fire automatically:
...
| `detect-gaps.sh` | SessionStart | Session begins | Detects fresh projects (suggests /game start) and missing documentation when code/prototypes exist, suggests /reverse-document or /project-stage-detect |
...
| `post-compact.sh` | PostCompact | After compaction | Reminds the agent to restore session state from `active.md` checkpoint |
...
| `validate-skill-change.sh` | PostToolUse (Write/Edit) | Skill file changes | Advises running `/skill-test` after any `assets/skills/` file is written or edited |

Hook reference documentation: `assets/docs/hooks-reference/`
Hook input schema documentation: `assets/docs/hooks-reference/hook-input-schemas.md`

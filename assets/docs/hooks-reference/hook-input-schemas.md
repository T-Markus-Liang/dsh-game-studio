...
This documents the JSON payloads each DSH hook receives on stdin for every event type.
...
Fired when a DSH session begins. **No stdin input** — the hook just runs and its stdout is shown to the agent as context.
...
Fired when the DSH session ends. **No stdin input** — the hook runs for cleanup and logging.
...
| 2 | Block (stderr shown to the agent) | PreToolUse only |

...
/** 通用 JSON 输出 schema（lossless JSON 节点） */
    ...
    execute: async ({ action, scope = '', logPath, filter = '', offset = 0 }, exec) => {
      ...
      switch (action) {
        ...
        case 'logs': {
          ...
          if (!logPath.startsWith(dir)) return { ok: false, error: 'logPath must be inside .dsh/game-studio/logs.' }
...
function gateTool(getConfig) {
  return {
    ...
    execute: async ({ action, gates = [], stepResult = null, runVerifier = false }, exec) => {
      ...
      if (next) writeActiveTask(cwd, { ...readActiveTask(cwd), phase: 'GATE', next })

...
/**
 * 进程级单实例守卫的全局注册表键（兼容性问题 0002：双加载）。
 *
 * 为什么用 `globalThis[Symbol.for(...)]` 而不是模块级 Map/WeakSet：
 * link 安装与 bundle 安装可能通过不同 specifier / 不同 node_modules 路径
 * 加载同一入口文件的**两份独立模块实例**，各自的模块级变量互不可见；
 * `Symbol.for` 走进程级 symbol registry，保证跨模块副本命中同一个键。
 */
...
export function apply(ctx, config = {}) {
  // ── 单实例守卫（兼容性问题 0002）────────────────────────────
  // 必须在 apply 同步段、任何其他注册之前：第二实例整体 no-op，
  // 第一实例的清理链保持完整。
  // 守卫依赖 ctx.effect 在 fiber dispose 时回调 cleanup 来释放标记
  //（vendor/cordis/src/fiber.ts:402-441：effect 收集的 disposer 在返回的
  // disposer 被调用或 fiber unload 时逆序执行，二者取先，重复调用为 no-op）。
  // 真实 Cordis Context 恒有 effect；若宿主（如极简测试 mock）未提供 effect，
  // 则无法保证卸载时释放标记，守卫降级为不启用——宁可不拦截也不留永久标记。
  if (typeof ctx.effect === 'function') {
    ...
    if (existing && existing.token !== token) {
      ...
      warn(
        '[dsh-game-studio] 检测到双加载（兼容性问题 0002）：本插件已有一个活动实例，'
        + '本次加载将被整体拒绝（不注册任何命令/工具/skills/设置/hooks/section）。'
        + '常见原因：同一插件同时出现在 `dsh.profile.bundles` 列表和手动 `cordis.patch.yml` insert 中——'
        + '两种安装路径二选一，不可混用。'
        + '详见 docs/compatibility/0002-duplicate-bundle-insert.md。',
      )
      return
    }
    globalThis[ACTIVE_KEY] = { token }
    ctx.effect(() => {
      return () => {
        // token 比对：只有仍持有标记的实例才允许清除，
        // 防止后启动实例（被拒或竞态）的残留 cleanup 误清前者的标记。
        if (globalThis[ACTIVE_KEY]?.token === token) {
          delete globalThis[ACTIVE_KEY]
        }
      }
    }, 'dsh-game-studio: single-instance guard')
  }
...
请完成上述任务，按给定 JSON Schema 输出结构化结果。`
      ...
      try {
        run = await subagents.start('spawn', {
          ...
          ...(agentOptionsFor(role, agent, getConfig().models, exec?.agent?.ctx) ? { agentOptions: agentOptionsFor(role, agent, getConfig().models, exec?.agent?.ctx) } : {}),
          ...
          if (parent && subagents && repairAgent) {
            ...
            try {
              run = await subagents.start('spawn', {
                ...
                ...(agentOptionsFor('specialist', repairAgent, getConfig().models, exec?.agent?.ctx) ? { agentOptions: agentOptionsFor('specialist', repairAgent, getConfig().models, exec?.agent?.ctx) } : {}),
...
export function agentOptionsFor(role, agent, models = {}, ctx = undefined) {
  const key = role === 'verifier' ? 'verifier' : agent?.modelTier === 'S' ? 'lead' : 'specialist'
  const configured = models[key]
  if (!configured || typeof configured !== 'object') {
    if (ctx) {
      const orchestrator = ctx.get?.('subagentOrchestrator')
      if (orchestrator?.resolveAgentOptions) {
        const resolved = orchestrator.resolveAgentOptions(key)
